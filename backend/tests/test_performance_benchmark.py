...
class TestRAGPerformance:
    ...
    @pytest.mark.asyncio
    async def test_rag_query_response_time_under_3s(self, monkeypatch):
        ...
        with (
            ...
            with (
                ...
                patch.object(
                    service.reranking_service, "rerank_results"
                ) as mock_rerank,
    ...
    @pytest.mark.asyncio
    async def test_rag_pipeline_step_timings(self, monkeypatch):
        ...
        with (
            ...
            with (
                ...
                patch.object(
                    ...
                    "search_similar",
                    side_effect=mock_search_with_timing,
                ),
                patch.object(
                    service.reranking_service,
                    "rerank_results",
                    side_effect=mock_rerank_with_timing,
                ),
                patch.object(
                    service, "_generate_answer", side_effect=mock_answer_with_timing
                ),
            ):
    ...
    @pytest.mark.asyncio
    async def test_concurrent_requests_stability(self, monkeypatch):
        ...
        with (
            ...
            with (
                ...
                tasks = [
                    service.query(query=f"Question {i}", topic_ids=[1])
                    for i in range(10)
                ]
                ...
                assert (
                    len(successful) == 10
                ), f"Only {len(successful)}/10 requests succeeded"
                assert (
                    elapsed < 5.0
